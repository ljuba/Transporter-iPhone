...
- (void)fetchVehiclesForDirection:(Direction *)direction {
	...
	CXMLDocument *vehiclesDocument = [[[CXMLDocument alloc] initWithContentsOfURL:url options:0 error:nil] autorelease];
	[url release];
	
	//stores the vehicle objects that are created from the XML data
	NSMutableArray *vehicles = [[[NSMutableArray alloc] init] autorelease];
	...
	NSMutableArray *filteredvehicles = [[[NSMutableArray alloc] init] autorelease];
	...
	for (NSDictionary *vehicle in vehicles) {
		...
		if ([[vehicle objectForKey:@"predictable"] isEqualToString:@"true"]) {
			...
			if ([vehicleRouteTag isEqualToString:route.tag]) {
				...
				if ([vehicleDirection.name isEqualToString:direction.name] && [vehicleDirection.title isEqualToString:direction.title]) {
					
					[filteredvehicles addObject:vehicle];
	...
	if([navController.topViewController respondsToSelector:@selector(didReceiveVehicles:)]){
		
		[navController.topViewController performSelectorOnMainThread:@selector(didReceiveVehicles:) withObject:filteredvehicles waitUntilDone:YES];
