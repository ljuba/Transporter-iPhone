...
- (id) init {
	...
	if (self = [super init]) {
		...
		self.directionTitleLabel = [[UILabel alloc] initWithFrame:CGRectMake(37, 32, 197, 25)];
		self.directionTitleLabel.font = [UIFont boldSystemFontOfSize:14];
		self.directionTitleLabel.textColor = [UIColor colorWithRed:0.147 green:0.147 blue:0.147 alpha:1.0];
		self.directionTitleLabel.adjustsFontSizeToFitWidth = YES;
		self.directionTitleLabel.text = @"";
		self.directionTitleLabel.backgroundColor = [UIColor colorWithWhite:0 alpha:0];
...
- (void) toggleFavorite {
	...
	if (isFavorite) {
		...
		if ([FavoritesManager removeStopFromFavorites:stop forLine:self.direction]) {
			isFavorite = NO;
	...
	if ([FavoritesManager addStopToFavorites:stop forLine:self.direction]) {
		isFavorite = YES;
...
- (void) setFavoriteStatus {
	...
	if ([FavoritesManager isFavoriteStop:stop forLine:self.direction]) {
		[favoriteButton setImage:[UIImage imageNamed:@"star-selected.png"] forState:UIControlStateNormal];
		[favoriteButton setImage:[UIImage imageNamed:@"star-selected.png"] forState:UIControlStateHighlighted];
		self.isFavorite = YES;
	} else {
...
- (void) setCellStatus:(int)status withArrivals:(NSArray *)arrivals {
	...
	if (cellStatus == kCellStatusDefault) {
		...
		case 0:
			self.directionTitleLabel.textColor = [UIColor colorWithRed:0.147 green:0.147 blue:0.147 alpha:0.6];
			break;
		case 1:
