...
- (void)viewDidLoad {
	...
	TripOverviewMapDelegate *mapDelegate = [[TripOverviewMapDelegate alloc] init];
...
- (void)showNextTrip {
	...
	if (previousSelectedTripIndex+1 == [trips count]-1 ) {
		topBar.nextTripButton.enabled = NO;
	}
	else {
		topBar.nextTripButton.enabled = YES;
	}
...
- (void)showPreviousTrip {
	...
	if (previousSelectedTripIndex-1 == 0 ) {
		topBar.previousTripButton.enabled = NO;
	}
	else {
		topBar.previousTripButton.enabled = YES;
...
- (void)routeTrip {
	...
	if ([startFieldText isEqualToString:@"Current Location"]) {
		...
		[tripRequest setValue:startFieldText forKey:@"start"];
	}
	...
	if ([endFieldText isEqualToString:@"Current Location"]) {
		...
		[tripRequest setValue:endFieldText forKey:@"end"];
	}
...
- (void)setRouteButtonStatus {
	...
	if ([startField.text length] > 1 && [endField.text length] > 1) {
