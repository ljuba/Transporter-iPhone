...
- (void)setTrip:(Trip *)_trip {
	...
	for (int i = 0; i <= numberOfLegs-1; i++) {
		...
		if ([leg isMemberOfClass:[WalkingLeg class]]) {
			...
			icon.image = [UIImage imageNamed:@"walking-icon.png"];
			...
			if ([agencyShortTitle isEqualToString:@"bart"]) {
				icon.image = [UIImage imageNamed:@"bart-icon.png"];
			}
			else if ([agencyShortTitle isEqualToString:@"sf-muni"]) {
				
				Route *route = [[transitLeg.startStop.directions anyObject] route];
				
				if ([route.vehicle isEqualToString:@"bus"]) {
					icon.image = [UIImage imageNamed:@"bus-icon.png"];	
				}
				else {
					icon.image = [UIImage imageNamed:@"rail-icon.png"];
				}
			}
			else {
				icon.image = [UIImage imageNamed:@"bus-icon.png"];
