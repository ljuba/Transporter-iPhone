...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (cell == nil) {
		cell = [[[RoutesCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:SFMuniRoutesCellIdentifier] autorelease];
	}
	...
	for (Route *route in rowRoutes) {
		...
		if (section == 1) {
			...
			if ([route.title isEqualToString:@"PowllMason Cable"]) {
				buttonTitle = @"Powell Mason";
			}
			else if ([route.title isEqualToString:@"PowellHyde Cable"]) {
				buttonTitle = @"Powell Hyde";
			}
			else if ([route.title isEqualToString:@"Calif. Cable Car"]) {
				buttonTitle = @"California";
			}
			else {
				buttonTitle = [route.title stringByReplacingOccurrencesOfString:@" Cable Car" withString:@""];
			}
