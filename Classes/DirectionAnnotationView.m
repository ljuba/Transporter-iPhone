...
- (void)setPoint:(CGPoint)point {
	...
	else if ((pinX + calloutViewWidth/2) > (mapFrame.size.width - kMapInset)) {
		
		deltaX = (mapFrame.size.width-kMapInset)-(pinX+calloutViewWidth/2);
...
- (void)setDirection:(Direction *)_direction {
	...
	if ([agencyShortTitle isEqualToString:@"actransit"]) {
		...
		pinView.image = [UIImage imageNamed:@"direction-pin-actransit.png"];
		
	}
	else {

		pinView.image = [UIImage imageNamed:@"direction-pin-sfmuni.png"];
