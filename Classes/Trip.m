...
- (void)processData {
	...
	for (id leg in legs) {
		...
		if ([leg isMemberOfClass:[WalkingLeg class]]) {
			...
			if (legIndex != 0 && legIndex != [legs count]-1 ) {
				
				[finalLegs removeLastObject];
...
- (void)printDescription {
	...
	for (id leg in legs) {
		...
		if ([leg isMemberOfClass:[WalkingLeg class]]) {
			...
			if (transitLeg.directions != nil && [transitLeg.directions count] > 0) {
				NSLog(@"  Direction: %@", [(Direction *)[transitLeg.directions objectAtIndex:0] tag]); /* DEBUG LOG */				
			}
