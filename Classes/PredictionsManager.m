...
+ (NSString *) arrivalsKeyForDirectionTag:(NSString *)dirTag inRoute:(Route *)route {
	...
	for (Direction *direction in _route.directions)
		...
		if ([direction.tag isEqualToString:_dirTag]) return([self arrivalsKeyForDirection:direction]);
	return(nil);
}
...
+ (Direction *) directionWithArrivalsKey:(NSString *)key inRoute:(Route *)route {
	...
	for (Direction *direction in _route.directions)
		...
		if ([direction.name isEqualToString:directionName]&&[direction.title isEqualToString:directionTitle]) return(direction);
	return(nil);
...
+ (NSDictionary *) filterPredictions:(NSDictionary *)predictions ForStop:(Stop *)stop {
	...
	for (NSString *key in filteredKeys) [filteredPredictions setObject:[predictions objectForKey:key] forKey:key];

	if ([filteredPredictions count] == 0) return(nil);
