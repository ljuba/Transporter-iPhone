...
- (void)viewDidLoad {
	...
	[FlurryAnalytics logEvent:@"LiveRoute - viewDidLoad" withParameters:nil];
...
- (void)didReceiveVehicles:(NSMutableArray *)vehicles {
	...
	if ([vehicles count] == 0) {
		...
		[FlurryAnalytics logEvent:@"LiveRoute - Not On Bus" withParameters:[DataHelper dictionaryFromRoute:direction.route]];
	...
	[FlurryAnalytics logEvent:@"LiveRoute - On Bus" withParameters:[DataHelper dictionaryFromRoute:direction.route]];
...
- (NSString *)matchingVehicleID:(NSMutableArray *)vehicles {
	...
	if (closestVehicleDistance < 600) {
		return [closestVehicle objectForKey:@"id"];
	}
	else {
		return nil;
	}
...
- (void)didReceivePredictions:(NSMutableDictionary *)_predictions {
	...
	if ([_predictions objectForKey:predictionKey] == nil) {
		return;
	}
	...
	for (NSArray *directionArrivals in [prediction.arrivals allValues]) {
		...
		for (NSDictionary *arrival in directionArrivals) {
			[filteredArrivals addObject:arrival];
		}
	...
	if ([filteredArrivals count] == 0) {
		...
		[FlurryAnalytics logEvent:@"LiveRoute - ETA Fail" withParameters:[DataHelper dictionaryFromRoute:direction.route]];
	...
	[FlurryAnalytics logEvent:@"LiveRoute - ETA Successful" withParameters:[DataHelper dictionaryFromRoute:direction.route]];
...
- (void)giveupLocationFix {
	...
	[FlurryAnalytics logEvent:@"LiveRoute - Giveup Location Fix" withParameters:[DataHelper dictionaryFromRoute:direction.route]];
...
- (void)positionUserMarkerToNewLocation:(CLLocation *)newLocation fromLocation:(CLLocation *)oldLocation {
	...
	if (label.text == kLiveRouteFindingLocation) {
		...
		if (!isBART) {
			...
			label.text = kLiveRouteBARTMessage;
		}
	}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	if (stopIndex == 0) {
		cell.imageView.image = [UIImage imageNamed:[NSString stringWithFormat:@"stop-beginning-route-%@.png", agencyShortTitle]];
	}
	else if (stopIndex == [stops count]-1) {
		cell.imageView.image = [UIImage imageNamed:[NSString stringWithFormat:@"stop-end-route-%@.png", agencyShortTitle]];
	}
	else {
		cell.imageView.image = [UIImage imageNamed:[NSString stringWithFormat:@"stop-mid-route-%@.png", agencyShortTitle]];
	}
