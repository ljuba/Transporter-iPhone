...
+ (BOOL)isFavoriteStop:(Stop *)stop forLine:(id)line {
	...
	if ([line isMemberOfClass:[Direction class]]) {
	...
	if (direction != nil) {
		agencyShortTitle = [NSString stringWithString:direction.route.agency.shortTitle];
	}
	else {
...
+ (BOOL)addStopToFavorites:(Stop *)stop forLine:(id)line {
	...
	[FlurryAnalytics logEvent:@"Stop Details - Add Stop" withParameters:[DataHelper dictionaryFromStop:stop]];
...
+ (BOOL)removeStopFromFavorites:(Stop *)stop forLine:(id)line; {
	...
	[FlurryAnalytics logEvent:@"Stop Details - Remove Stop" withParameters:[DataHelper dictionaryFromStop:stop]];
